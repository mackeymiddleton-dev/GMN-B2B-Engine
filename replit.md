...
**Server-side belt-and-suspenders for #1 (added same patch):** `server.js` line ~1278 — when `[BOOKED]` is emitted but `fresh.researchData` OR `fresh.scanResults` is missing, the marker is stripped, the AI is NOT paused, and the conversation continues. This catches the rare case where the model drifts past the prompt-level rule. Logs the suppression with `[AiGen] [BOOKED] suppressed for ${contactId} (premature: ...)`. **Canonical asymmetry:** the canonical `conversationPrompt` predates the NEVER BOOK BEFORE QUALIFYING block and does NOT carry it (A/B/C/D were retrofitted with the block + EXCEPTION carve-out paragraph; canonical is 23K vs ~30K for the variants). This is INTENTIONAL — canonical is the historical baseline and the server-side gate above is the safety net for any contact still on canonical. Do not "fix" by copy-pasting the block into canonical without first auditing whether canonical is still in active rotation; if it is, decide explicitly whether to retire canonical or formally retrofit it.
